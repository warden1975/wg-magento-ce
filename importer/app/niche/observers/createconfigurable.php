...
class createconfigurable implements interfaces\observer
{
    ...
    protected function createConfigurable($value)
    {
        ...
        $configurable['sku'] = strtolower(trim($value['namekey']));
        $configurable['has_options'] = 1;
        $configurable['qty'] = 0;
        $configurable['type'] = $configurable['product_type_id'] = 'configurable';
        $configurable['qty'] = 0;
        $configurable['niche_entity_id'] = 'configurable_niche_id_' . $configurable['sku'];

        // attrs that should be empty
        $empty_attrs = array('color', 'colour_filter', 'size', 'dsizes', 'ink_color', 'cover', 'source', 'ruling', 'style', 'use', 'point_width', 'suits', 'dpages', 'dstyle', 'dcover', 'dspecs', 'dcable', 'dquantity', 'dink', 'dtip', 'dmaterial', 'ddate', 'dcatalogue', 'dsuit', 'dformat', 'dzip', 'dfeature', 'dcontain', 'dhold', 'dlength', 'dheight', 'dnote', 'dextra', 'shelf_location', 'barcodes');
    ...
    public function run($configurable)
    {
        ...
        while(list($key, $simple_product) = each($rows)) {
            ...
            if (!$this->hasConfigurable($simple_product['namekey'])) {
                ...
                if ($rows[$key]['type'] == 'simple') {
                    $rows[$key]['visibility'] = 1;
                }
            }
            ...
            $configurable_product['status'] = $this->getStatus($simple_product['namekey']);
    ...
    public function getStatus($namekey)
    {
